<!-- Navbar Section Start -->
<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="icon" type="image/png" href="#" alt="Banjo Bob logo" />
    <title>Banjo Lessons with Banjo Bob | Personalized Banjo Lessons in Parrish, FL</title>
    <link rel="shortcut icon" type="image/x-icon" href="data:image/png;base64," />
    <!-- Bootstrap via Internet -->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous">
    <link rel="stylesheet" type="text/css" href="/css/index.css" />
</head>

<body class="bg-white">

    <?php
    include(__DIR__ . '/../config/constants.php');
    ?>

    <!-- Navbar Section Start -->
    <nav class="navbar navbar-expand-lg navbar-light bg-light fade-down">
        <div class="container-fluid">
            <a class="navbar-brand" href="<?php echo SITEURL; ?>">
                <img class="ms-2" src="data:image/png;base64,">
            </a>
            <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
                <span class="navbar-toggler-icon"></span>
            </button>
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
                <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                    <li class="nav-item">
                        <a class="nav-link active navbar-brand" aria-current="page" href="<?php echo SITEURL; ?>">
                            <?php echo $company_name; ?>
                        </a>
                    </li>
                </ul>
                <ul class="navbar-nav me-2 mb-2 mb-lg-0">
                    <li class="nav-item">
                        <a class="nav-link active" href="<?php echo SITEURL;?>pages/listen.php">Listen</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link active" href="<?php echo SITEURL;?>pages/lessons.php">Lessons</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link active" href="<?php echo SITEURL;?>pages/about.php">About</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link active" href="<?php echo SITEURL;?>pages/testimonials.php">Testimonials</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link active" href="<?php echo SITEURL;?>pages/contact.php">Contact</a>
                    </li>
                    </li>
                </ul>
            </div>
        </div>
    </nav>
    <!-- Navbar Section End -->